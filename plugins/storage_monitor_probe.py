...
# ########################################################################### #
app = nap.core.Plugin(
    description="NAGIOS Storage Free Space probe", version=PROBE_VERSION
)
...
app.add_argument(
    "-m", "--min_free_space", help="The min free space in GB ", default="100"
)
app.add_argument(
    "-O",
    "--ops_portal_url",
...
def get_storages(args, io):
    ...
    c.setopt(c.HTTPHEADER, ["accept: application/json", apikey_header])
    c.perform()
    c.close()
...
def metricCheckFreeSpace(args, io):
    ...
    try:
        ...
        if not found:
            io.summary = (
                "The information about the storage %s are not avaiable " % args.hostname
            )
            io.status = nap.CRITICAL
            return
        ...
        if freesize < int(args.min_free_space):
            io.summary = (
                "Free space is less than the threshold configured: %s GB"
                % str(freesize)
            )
            io.status = nap.CRITICAL
        elif freesize < threshold_with_percent:
            io.summary = (
                "Free space is close to the minimum threshold configured: %s GB "
                % str(freesize)
            )
